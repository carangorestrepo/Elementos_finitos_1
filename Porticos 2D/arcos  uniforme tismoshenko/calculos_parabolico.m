...
Vz=V*cos(atan(dydx)); %% cortante cargas verticales
P=V*sin(atan(dydx)); %% axial cargas verticales
%% esfuerzo cortante unitario

Vu=cos(atan(dydx));%% cortante unitario
Pu=sin(atan(dydx));%% axial unitario
%% Longitud funcion de arco 
dxdym=sqrt(1+dydx^2);

%% Cálculo de deformacion vertical
% Deformación vertical por cargas externas
AMx = matlabFunction((M * x / EI + Vz /Ac) * dxdym , "Vars", x);
% Deformación vertical por carga unitaria vertical
Axx = matlabFunction((x * x / EI+ Vu/Ac)* dxdym , "Vars", x);
% Deformación horizontal por carga unitaria vertical
Axy = matlabFunction((x * y  / EI + Pu/Ac)* dxdym, "Vars", x);
...
AMy = matlabFunction((M * y / EI + P /Ac) * dxdym, "Vars", x);
% Deformación horizontal por carga unitaria horizontal
Ayy = matlabFunction((y * y / EI + Pu/Ac)* dxdym , "Vars", x);
% Deformación vertical por carga unitaria horizontal
Ayx = matlabFunction((y * x / EI + Vu/Ac)* dxdym , "Vars", x);
