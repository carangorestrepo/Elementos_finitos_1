...
for e = 1:nef
   % se dibuja el EF e
   nod_ef = LaG(e,[1,4,5,2,6,7,3,8,9 1]);
...
N   = cell(nef,n_gl,3,2*10); % contenedor para las matrices de forma
B   = cell(nef,n_gl,3,2*10); % contenedor para las matrices de deformacion
...
for e = 1:nef
   ...
   idx{e}           = reshape(gdl(LaG(e,:),:)', 1, 10*2);
...
for i = 1:nlcd
   e     = idxELEM(i);
   LaG_e = LaG(e,:);
   fte   = t2ft_T10(xnod(LaG_e,[X Y]), LaG_e, nodoijk(i,:), carga(i,:), t(mat(e)));
...
hold on
for e = 1:nef
   line(xnod(LaG(e,[1,4,5,2,6,7,3,8,9 1]),X), xnod(LaG(e,[1,4,5,2,6,7,3,8,9 1]),Y), 'Color','b'); % original
   line(xdef(LaG(e,[1,4,5,2,6,7,3,8,9 1]),X), xdef(LaG(e,[1,4,5,2,6,7,3,8,9 1]),Y), 'Color','r'); % deformada
end
xlabel(['Eje X [' U_LONG ']']);
ylabel(['Eje Y [' U_LONG ']']);
axis equal tight;
