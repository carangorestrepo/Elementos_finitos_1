...
e_ = [ 7, 8, 9, 10, 11, 12 ];            % GDL a eliminar en condensaci�n nodal
...
Nforma6 = @(xi,eta)[
                    ...
                    -4*eta*(eta - 1); %%4,5
                    -4*xi*(xi - 1); %% 6, 4
                    - 4*eta^2 - 8*eta*xi + 4*eta - 4*xi^2 + 4*xi% 5 6
                    ];
 % programa deduccion_funciones_forma/FF_serendipitos_Q4_Q8.m
Nforma3 = @(xi,eta)[
                    1 - xi - eta
...
for e = 1:nef
   ...
   for p = 1:n_gl
         ...
         dx_dxi  = sum(ddN_dxi(1:3)  .* xe);   dy_dxi  = sum(ddN_dxi(1:3)  .* ye);
         dx_deta = sum(ddN_deta(1:3) .* xe);   dy_deta = sum(ddN_deta(1:3) .* ye);
         ...
         Ke12 = Ke12 + B{e,p}'*De{mat(e)}*B{e,p} * det_Je(p)*t(mat(e))*w_gl(p);
         fe = fe + N3{e,p}'*be{mat(e)}        * det_Je(p)*t(mat(e))*w_gl(p);
         Me12 = Me12 + N6{e,p}'*rho *N6{e,p}       * det_Je(p)*t(mat(e))*w_gl(p);
    end
