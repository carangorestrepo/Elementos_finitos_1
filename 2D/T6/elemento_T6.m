...
% programa deduccion_funciones_forma/FF_serendipitos_T10.m
 ...
 Nforma =  @(xi, eta) [(1 - (xi + eta))*(1 - 2*(xi + eta));
                                      xi*(2*xi - 1);
                                    eta*(2*eta - 1);
                              4*xi*(1 - (xi + eta));
                                           4*xi*eta;
...
for i = 1:nlcd
   e     = idxELEM(i);
   LaG_e = LaG(e,:);
   fte   = t2ft_T6(xnod(LaG_e,[X Y]), LaG_e, nodoijk(i,:), carga(i,:), t(mat(e)));
...
hold on
for e = 1:nef
   line(xnod(LaG(e,[1,4,2,5,3,6,1]),X), xnod(LaG(e,[1,4,2,5,3,6,1]),Y), 'Color','b'); % original
   line(xdef(LaG(e,[1,4,2,5,3,6,1]),X), xdef(LaG(e,[1,4,2,5,3,6,1]),Y), 'Color','r'); % deformada
end
xlabel(['Eje X [' U_LONG ']']);
ylabel(['Eje Y [' U_LONG ']']);
axis equal tight;
