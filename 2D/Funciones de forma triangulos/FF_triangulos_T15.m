...
xxi  = coord(:, X); 
eeta = coord(:, Y);
A = [ ones(nno,1) xxi eeta xxi.^2 xxi.*eeta eeta.^2 xxi.^3 xxi.^2.*eeta xxi.*eeta.^2 eeta.^3 xxi.^4 xxi.^3.*eeta xxi.^2.*eeta.^2 xxi.*eeta.^3 eeta.^4];
N = cell(nno,1);
syms xi eta
for i = 1:nno
   ...
   b = zeros(nno,1);   b(i) = 1;
   coef_alpha = A\b;
   N{i} = simplify([ 1 xi eta xi^2 xi*eta eta^2 xi^3 xi.^2*eta xi.*eta.^2 eta.^3 xi^4 xi^3*eta xi^2*eta^2 xi*eta^3 eta^4]*coef_alpha);
end
