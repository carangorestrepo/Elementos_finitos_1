...
r_ = [ 1, 2, 3, 4, 5, 6, 7, 8 ];   % GDL a retener en condensaci�n nodal
e_ = [ 9, 10, 11, 12 ];            % GDL a eliminar en condensaci�n nodal
...
Nforma4 = @(xi,eta)[ ((eta - 1)*(xi - 1))/4    % N1
                   -((eta - 1)*(xi + 1))/4    % N2
                    ((eta + 1)*(xi + 1))/4    % N3
                   -((eta + 1)*(xi - 1))/4 ]; % N4
%                           b   1 - xi**2,    % N5
...
n_gl         = 10; % orden de la cuadratura de Gauss-Legendre
...
inv_Mee = cell(nef, 4, 4);
Mer     = cell(nef, 4, 8);

N6   = cell(nef,n_gl,n_gl); % contenedor para las matrices de forma
N4   = cell(nef,n_gl,n_gl); % contenedor para las matrices de forma
...
for e = 1:nef
   ...
   %# se calcula el �rea del cuadrilatero (del EF)
   %# Ver https://en.wikipedia.org/wiki/Shoelace_formula
   %# Ae = 0.5*(x1y2 + x2y3 + x3y4 + x4y1 - x2y1 - x3y2 - x4y3 - x1y4)
    Ae = 0.5*(xe(1)*ye(2) + xe(2)*ye(3) + xe(3)*ye(4) + xe(4)*ye(1)... 
            - xe(2)*ye(1) - xe(3)*ye(2) - xe(4)*ye(3) - xe(1)*ye(4));
   for p = 1:n_gl
      for q = 1:n_gl
         xi_gl  = x_gl(p);
         eta_gl = x_gl(q);
         ...
         NNforma4  = Nforma4 (xi_gl, eta_gl);
         NNforma6  = Nforma6 (xi_gl, eta_gl);
         ...
         N4{e,p,q} = zeros(2, 2*4);
         N6{e,p,q} = zeros(2, 2*6);
         B{e,p,q} =  zeros(3, 2*6);
         for i = 1:4
            % Se ensambla la matriz de funciones de forma N
            N4{e,p,q}(:,[2*i-1 2*i]) = [ NNforma4(i)  0         
                                        0           NNforma4(i) ];
         end
         for i = 1:6
                        % Se ensambla la matriz de funciones de forma N
            N6{e,p,q}(:,[2*i-1 2*i]) = [ NNforma6(i)  0         
                                        0           NNforma6(i) ];
   ...
   for p = 1:n_gl
      for q = 1:n_gl
          B56c = B56c -(1/Ae) * B{e,p,q}(:,9:12)* det_Je(p,q)*t(mat(e))*w_gl(p)*w_gl(q);
      end
   end
   for p = 1:n_gl 
      for q = 1:n_gl 
         B{e,p,q}(:,9:12) =B{e,p,q}(:,9:12)+B56c;
         ...
         Ke12 = Ke12 + B{e,p,q}'*De{mat(e)}*B{e,p,q} * det_Je(p,q)*t(mat(e))*w_gl(p)*w_gl(q);
         fe = fe + N4{e,p,q}'*be{mat(e)}         * det_Je(p,q)*t(mat(e))*w_gl(p)*w_gl(q);
         Me12 = Me12 + N6{e,p,q}'*rho *N6{e,p,q}     * det_Je(p,q)*t(mat(e))*w_gl(p)*w_gl(q);
      end
   end
...
for i = 1:nlcd
   e     = idxELEM(i);
   LaG_e = LaG(e,:);
   fte   = t2ft_Q6(xnod(LaG_e,[X Y]), LaG_e, nodoijk(i,:), carga(i,:), t(mat(e)));
