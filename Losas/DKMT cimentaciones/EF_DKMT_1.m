...
for e = 1:nef               % ciclo sobre todos los elementos finitos
    ...
    for pp = 1:n_gl
       % for qq = 1:n_gl           
           ...
           %% Se calcula la matriz de deformacion por flexion Bb (eq. 41)
            ...
            He= He + N{e,pp}'*N{e,pp}*h*Balastro*det_Je(pp)*w_gl(pp);
            %% vector de fuerzas nodales equivalentes        
            %if (xe(1) >= cx0 && xe(2) <= (cx0+lcx)) && ...
            %   (ye(2) >= cy0 && ye(3) <= (cy0+lcy))
                fe = fe + N{e,pp}'*[q(e) 0 0]'*det_Je(pp)*w_gl(pp);
            %end
        %end
    end
    factor=1;
    ...
    K(idx{e},idx{e}) = K(idx{e},idx{e}) + (Kbe + Kse)*factor + He;
...
function plot_M_or_Q(nef, xnod, LaG, variable, texto, angulos)
    ...
    if nargin == 6
        step = 3; % reducción densidad flechas
        for i = 1:length(angulos)
            quiver(xnod(1:step:end,X),xnod(1:step:end,Y),...
                   cos(angulos{i}(1:step:end)),...
                   sin(angulos{i}(1:step:end)),...
                   esc,'k','ShowArrowHead','off','LineWidth',1);
        end
    end
end
