...
',densidad_columna);
fprintf('Numero nodos: %d
',size(xnod,1));
fprintf('Numero elementos: %d
',size(LaG,1));
fprintf('Area mallada: %.8f m^2
',sum(area_elem));
fprintf('Area teorica: %.8f m^2
',Lx*Ly);
...
fprintf('
VERIFICACION DE ACCIONES DE COLUMNA
');
fprintf('P objetivo/integrado = %.8f / %.8f kN
',P,Pcheck);
fprintf('Mx objetivo/integrado = %.8f / %.8f kN*m
',Mx,Mxcheck);
fprintf('My objetivo/integrado = %.8f / %.8f kN*m
',My,Mycheck);
