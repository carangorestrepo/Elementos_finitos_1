...
hold on;
for e = 1:nef
   line(xnod(LaG(e,[1,4,2,5,3,6,1]),X), xnod(LaG(e,[1,4,2,5,3,6,1]),Y));
...
for e = 1:nef      % ciclo sobre todos los elementos finitos
   ...
   Kbe = zeros(3*nnoef);
   Kse = zeros(3*nnoef); 
   det_Je = zeros(n_gl_b,1); % Jacobianos con n_gl puntos de integracion   
   for p = 1:n_gl_b
      %for q = 1:n_gl
         xi_gl  = x_gl_b(p);
         eta_gl = e_gl_b(p);
   ...
   for p = 1:n_gl_b
      %for q = 1:n_gl_b
         ...
         Mbe = Mbe + NN{e,p}'*NN{e,p}*det_Je(p)*w_gl_b(p);                                              % REVISAR !!!!!!!!!!!!!!!!   
      %end
      %p
   end  
...
A=[1.66666666666668,-0.333333333333340,-0.333333333333340;
    ...
    -2.33333333333332,3.66666666666666,-0.333333333333340;
    0.666666666666680,-0.333333333333340,0.666666666666660;
    -1.33333333333332,1.66666666666666,0.666666666666660;
    -0.333333333333320,1.66666666666666,-0.333333333333340];

for e = 1:nef
   sigmag_bb=[sigmag_b{e,:}]; 
   Mx(LaG(e,:),:)  = Mx(LaG(e,:),:)  + A * sigmag_bb(1,:)';

   My(LaG(e,:),:)  = My(LaG(e,:),:)  + A *  sigmag_bb(2,:)';

   Mxy(LaG(e,:),:) = Mxy(LaG(e,:),:) + A *  sigmag_bb(3,:)';
   ...
   switch n_gl_s
     ...
     case 2
       ...
       Qx(LaG(e,:),:)  = Qx(LaG(e,:),:)  + A * sigmag_ss(1,:)';

       Qy(LaG(e,:),:)  = Qy(LaG(e,:),:)  + A * sigmag_ss(2,:)';
   end                                         
